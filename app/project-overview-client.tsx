    ...
    <section className="mt-5">
      ...
      {projects.length === 0 ? (
        ...
        {visibleProjects.map((project) => <article key={project.code} className="flex flex-col gap-4 px-4 py-4 transition-colors hover:bg-slate-50 sm:px-5 lg:flex-row lg:items-center">
          <div className="flex min-w-0 flex-1 items-start gap-3"><a href={`/projetos/detalhe?codigo=${encodeURIComponent(project.code)}`} className="mt-0.5 grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-[#edf2fb] text-[#103f85]" aria-label={`Abrir pasta de ${project.name}`}><Folder size={21} fill={project.color} /></a><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="truncate text-base font-bold text-slate-800"><a href={`/projetos/detalhe?codigo=${encodeURIComponent(project.code)}`} className="hover:text-[#103f85] hover:underline">{project.name}</a></h3><span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: project.color }} /></div><p className="mt-1 text-xs font-semibold text-slate-500">{project.code} · {project.scheduleId ? 'Cronograma criado' : 'Cronograma pendente'}</p><a href={`/projetos/detalhe?codigo=${encodeURIComponent(project.code)}`} className="mt-1 inline-block text-xs font-semibold text-[#103f85] hover:underline">Abrir pasta e premissas</a></div></div>
