...
export async function findProjectByName(name: string) {
  ...
  const row = await getDb()
    .prepare('SELECT id, code, name, color, created_at, updated_at FROM pmo_projects WHERE lower(name) = lower(?)')
    .bind(normalized)
    .first<Record<string, unknown>>();
  return row ? {
    id: String(row.id), code: String(row.code), name: String(row.name),
    color: String(row.color || fallbackColor), createdAt: String(row.created_at || ''), updatedAt: String(row.updated_at || ''),
...
export async function ensureProjectRecord(name: string, color = fallbackColor) {
  ...
  const record: ProjectRecord = {
    id: crypto.randomUUID(), code: await nextProjectCode(), name: normalized, color,
    createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
  };
  await getDb().prepare('INSERT INTO pmo_projects (id, code, name, color, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)')
    .bind(record.id, record.code, record.name, record.color, record.createdAt, record.updatedAt).run();
  return record;
}
...
export async function readProjectOverview() {
  ...
  const [projects, schedules, actionCounts] = await Promise.all([
    db.prepare('SELECT id, code, name, color, created_at, updated_at FROM pmo_projects ORDER BY created_at DESC, name').all(),
  ...
  return projects.results
    .map((project) => {
      ...
      return {
        id: String(project.id), code, name: String(project.name), color: String(project.color || fallbackColor), createdAt: String(project.created_at || ''),
