...
export async function syncScheduleActions(projectId: string, projectName: string) {
  const db = getDb();
  ...
  const statements = tasks.results.flatMap((row) => {
    ...
    return [
      ...
      db.prepare("INSERT INTO postit_actions (id, title, observation, owner, action_date, board_day, sector, project, status, criticality, completed, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'Médio', ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) ON CONFLICT(id) DO UPDATE SET title = excluded.title, observation = excluded.observation, owner = excluded.owner, action_date = excluded.action_date, board_day = excluded.board_day, sector = excluded.sector, project = excluded.project, status = excluded.status, completed = excluded.completed, updated_at = CURRENT_TIMESTAMP").bind(id, String(row.title), String(row.observation || ''), owner, date, boardDayFor(date), sector, projectName, Number(row.progress) >= 100 ? 'Concluído' : 'No prazo', Number(row.progress) >= 100 ? 1 : 0),
    ];
  });
